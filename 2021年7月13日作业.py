...
class Qping():
    ...
    def make_pkt(self):
        if self.srcip:
            self.pkt = IP(dst=self.ipaddr, src=self.srcip) / ICMP() / b'v' * self.length
        else:
            self.pkt = IP(dst=self.ipaddr) / ICMP() / b'v' * self.length
    ...
    def ping_five(self):
        self.make_pkt()
        for i in range(5):
            ret = sr1(self.pkt, timeout=2, verbose=False)
            if ret:
    ...
    def __str__(self):
        if self.srcip is None:
            ...
            return '<%5s=>dest_ip: %5s,src_ip:%5s,size: %s>' % (
                self.__class__.__name__, self.ipaddr, self.srcip, self.length)
...
class Newping(Qping):
    def ping_five(self):
        self.make_pkt()
        for i in range(5):
            ret = sr1(self.pkt, timeout=1, verbose=False)
            # print(self.ping_pkt.show())
            if ret:
                print('!', end='', flush=True)
            else:
